...
export default {
  async fetch(req, env, ctx) {
    ...
    // Matomo Live proxy (existing feature)
    if (url.pathname === "/live") {
      ...
      return new Response(JSON.stringify(j), { headers: { "content-type":"application/json; charset=UTF-8", "access-control-allow-origin":"*" } })
    }

    // Simple live widget page
    ...
    if (url.pathname === "/reviews") {
      if (req.method === "GET") {
        ...
        return new Response(JSON.stringify({ avg: Math.round(avg*10)/10, count: items.length, items: items.slice(-20).reverse() }), { headers: { "content-type":"application/json; charset=UTF-8","access-control-allow-origin":"*" } })
      }
      if (req.method === "POST") {
        const body = await req.json()
        const item = { name: (body.name||"Гость").toString().slice(0,48), text: (body.text||"").toString().slice(0,1000), rating: Number(body.rating)||5, at: new Date().toISOString() }
        const raw = await env.REVIEWS.get("items")
        const items = raw ? JSON.parse(raw) : []
        items.push(item)
        await env.REVIEWS.put("items", JSON.stringify(items))
        return new Response(JSON.stringify({ ok:true }), { headers: { "content-type":"application/json; charset=UTF-8","access-control-allow-origin":"*" } })
      }
      return new Response(JSON.stringify({error:"method_not_allowed"}), { status:405, headers: { "content-type":"application/json; charset=UTF-8","access-control-allow-origin":"*" } })
    }
